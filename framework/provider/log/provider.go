...
import (
	...
	"github.com/ryanmoriarty-lee/yoimiya-gen/framework/provider/log/services"
	"io"
	"strings"
)

// YoimiyaLogServiceProvider 服务提供者
type YoimiyaLogServiceProvider struct {
...
// Register 注册一个服务实例
func (l *YoimiyaLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		tcs, err := c.Make(contract.ConfigKey)
		if err != nil {
			// 默认使用console
			return services.NewYoimiyaConsoleLog
		}
	...
	switch l.Driver {
	case "single":
		return services.NewYoimiyaSingleLog
	case "rotate":
		return services.NewYoimiyaRotateLog
	case "console":
		return services.NewYoimiyaConsoleLog
	case "custom":
		return services.NewYoimiyaCustomLog
	default:
		return services.NewYoimiyaConsoleLog
	}
}
...
// Boot 启动的时候注入
func (l *YoimiyaLogServiceProvider) Boot(c framework.Container) error {
	return nil
}

// IsDefer 是否延迟加载
func (l *YoimiyaLogServiceProvider) IsDefer() bool {
	return false
}

// Params 定义要传递给实例化方法的参数
func (l *YoimiyaLogServiceProvider) Params(c framework.Container) []interface{} {
	// 获取configService
...
// Name 定义对应的服务字符串凭证
func (l *YoimiyaLogServiceProvider) Name() string {
