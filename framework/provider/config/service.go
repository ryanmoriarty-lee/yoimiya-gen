...
// YoimiyaConfig  表示yoimiya框架的配置文件服务
type YoimiyaConfig struct {
...
// 读取某个配置文件
func (conf *YoimiyaConfig) loadConfigFile(folder string, file string) error {
	conf.lock.Lock()
...
// 删除文件的操作
func (conf *YoimiyaConfig) removeConfigFile(folder string, file string) error {
	conf.lock.Lock()
	defer conf.lock.Unlock()
	s := strings.Split(file, ".")
	// 只有yaml或者yml后缀才执行
...
// NewYoimiyaConfig 初始化Config方法
func NewYoimiyaConfig(params ...interface{}) (interface{}, error) {
	...
	for _, file := range files {
		fileName := file.Name()
		err := yoimiyaConf.loadConfigFile(envFolder, fileName)
		if err != nil {
			log.Println(err)
			continue
		}
			...
			case ev := <-watch.Events:
				{
					...
					if ev.Op&fsnotify.Create == fsnotify.Create {
						...
						yoimiyaConf.loadConfigFile(folder, fileName)
					}
					...
					if ev.Op&fsnotify.Remove == fsnotify.Remove {
						...
						yoimiyaConf.removeConfigFile(folder, fileName)
					}
				}
...
// 通过path来获取某个配置项
func (conf *YoimiyaConfig) find(key string) interface{} {
	conf.lock.RLock()
