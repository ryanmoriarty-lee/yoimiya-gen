...
var newCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		}
		{
			...
			prompt := &survey.Input{
				Message: "请输入版本名称(参考 https://github.com/ryanmoriarty-lee/yoimiya-gen/releases，默认为最新版本)：",
			}
		...
		filepath.Walk(folder, func(path string, info os.FileInfo, err error) error {
			...
			isContain := bytes.Contains(c, []byte("github.com/ryanmoriarty-lee/yoimiya-gen/app"))
			if isContain {
				fmt.Println("更新文件:" + path)
				c = bytes.ReplaceAll(c, []byte("github.com/ryanmoriarty-lee/yoimiya-gen/app"), []byte(mod+"/app"))
				err = ioutil.WriteFile(path, c, 0644)
				if err != nil {
					return err
				}
