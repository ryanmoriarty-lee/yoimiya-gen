...
var providerListCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		yoimiyaContainer := container.(*framework.YoimiyaContainer)
		// 获取字符串凭证
		list := yoimiyaContainer.NameList()
		// 打印
...
var providerCreateCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		// 检查服务是否存在
		providers := container.(*framework.YoimiyaContainer).NameList()
...
import "github.com/ryanmoriarty-lee/yoimiya-gen/framework"
